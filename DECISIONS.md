...
26. **Fully offline write queue over disable-only mode** — sprint 5 queues every transaction/category/budget/profile write to an AsyncStorage-persisted outbox and replays it FIFO on reconnect (`useOfflineMutation` + `useOfflineSync`). A disable-only approach would have lost user input and defeated the "offline-first" goal.
27. **Reactive currency via a dedicated Zustand store** — `currencyStore` + `useCurrency` reformat every screen instantly on currency change. Kept separate from `authStore` so formatting reacts independently while `setAuth`/`setUser` sync the user's `currencyCode` into it.
